...
public:
  ...
  /// sourcemeta::core::SchemaFrame::Mode::Root reports on a single schema,
  /// so framing a wrapper that holds more than one yields no locations
  ///
  /// sourcemeta::core::SchemaFrame::Mode::Locations and
  /// sourcemeta::core::SchemaFrame::Mode::References locate the schemas of
  /// the document rather than each of its JSON Pointers, and the latter also
  /// locates whatever place a reference names. Reach for
  /// sourcemeta::core::SchemaFrame::Mode::Pointers only to address a keyword
  /// or a value of the document by URI, as computing those locations tends to
  /// dominate the cost of framing
  enum class Mode : std::uint8_t {
    /// Report on the single schema that the document declares
    Root,
    /// Report on every schema resource and anchor of the document
    Locations,
    /// Also report on whatever place every reference names
    References,
    /// Also report on every JSON Pointer of the document
    Pointers
  };
  ...
  struct Reference {
    /// The reference exactly as the schema spells it
...
#pragma GCC diagnostic ignored "-Wshadow"
  ...
  enum class LocationType : std::uint8_t {
    /// A schema resource, as introduced by an identifier
    Resource,
    /// An anchor declared within a schema resource
    Anchor,
    /// A JSON Pointer into the document that is not a schema of its own
    Pointer,
    /// A subschema that declares no identifier of its own
    Subschema
  };
#if defined(__GNUC__)
#pragma GCC diagnostic pop
  ...
  /// A location entry
  struct Location {
    /// The location of the closest enclosing schema resource, if any
    std::optional<sourcemeta::core::WeakPointer> parent;
    /// What kind of place in the document this entry describes
    LocationType type;
    /// The base URI that this entry resolves relative references against
    std::string_view base;
    /// Where in the document this entry lives
    sourcemeta::core::WeakPointer pointer;
    /// How many tokens of the pointer fall inside the enclosing resource
    std::size_t relative_pointer;
    /// The meta-schema in effect at this entry
    std::string_view dialect;
    /// The base dialect in effect at this entry
    SchemaBaseDialect base_dialect;
    /// Whether this entry sits under a keyword that applies to property names
    bool property_name;
    /// Whether nothing in the document reaches this entry
    bool orphan;
  };
  ...
  /// throwing sourcemeta::core::SchemaFrameLimitError rather than analysing
  /// past it. What the limit buys depends on the mode, as
  /// sourcemeta::core::SchemaFrame::Mode::Pointers locates every JSON Pointer
  ...
  /// sourcemeta::core::SchemaFrame::root, this works for anonymous schemas
  ...
  /// sourcemeta::core::SchemaFrame::has_references_to, which matches a
