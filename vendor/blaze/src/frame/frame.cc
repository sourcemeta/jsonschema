  ...
  if (schema.is_object() &&
    ...
    if (anchor_2020 && anchor_2020->is_string()) {
      ...
      if (!is_valid_anchor_2020_12(anchor_view)) {
        throw sourcemeta::blaze::SchemaKeywordError("$anchor", anchor_view,
  ...
  if (schema.is_object() &&
    ...
    if (id_value) {
      ...
      if (id_view.starts_with('#') && id_view.size() > 1) {
        ...
        if (!is_valid_anchor(anchor_view)) {
          throw sourcemeta::blaze::SchemaKeywordError("$id", id_view,
                                                      "Invalid anchor value");
        }
...
auto throw_already_exists(const sourcemeta::core::JSON::String &uri) -> void {
  throw sourcemeta::blaze::SchemaFrameError(uri,
...
auto SchemaFrame::to_json(
  ...
  for (const auto &location : this->locations_) {
    ...
    entry.assign_assume_new("type",
                            sourcemeta::blaze::to_json(location.second.type));
  ...
  for (const auto &reference : this->references_) {
    auto entry{sourcemeta::core::JSON::make_object()};
    entry.assign_assume_new("type",
                            sourcemeta::blaze::to_json(reference.first.first));
    entry.assign_assume_new(
      ...
      if (!maybe_id.empty()) {
        try {
          ...
          throw SchemaKeywordError(
              sourcemeta::blaze::id_keyword(root_base_dialect.value()),
    ...
    if (entry.common.subschema.get().is_object()) {
      ...
      if (ref_value) {
        if (!ref_value->is_string()) {
          ...
          throw sourcemeta::blaze::SchemaKeywordError(
              "$ref", value.str(), "Invalid reference value");
        }
