...
struct LocalCluster {
  ...
  LocalCluster(SocketContextOptions options = {},
               std::function<void(uWS::SSLApp &)> cb = nullptr) {
    std::transform(
        threads.begin(), threads.end(), threads.begin(),
        [options, &cb](std::thread *) {
          return new std::thread([options, &cb]() {
            ...
            app->preOpen(
                [](struct us_socket_context_t *context,
                  ...
                  // std::cout << "About to load balance " << fd << " to " <<
                  // roundRobin << std::endl;

                  auto receivingApp = apps[roundRobin];
                  apps[roundRobin]->getLoop()->defer(
                      [fd, receivingApp]() { receivingApp->adoptSocket(fd); });
