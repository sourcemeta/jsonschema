...
class CachingHttpResponse {
 public:
  ...
  void write(std::string_view data) { buffer.append(data); }
...
struct CachingApp : public uWS::TemplatedApp<SSL, CachingApp<SSL>> {
 public:
  CachingApp(SocketContextOptions options = {})
      : uWS::TemplatedApp<SSL, CachingApp<SSL>>(options) {}

  using uWS::TemplatedApp<SSL, CachingApp<SSL>>::get;

  CachingApp(const CachingApp &other) = delete;
  CachingApp(CachingApp<SSL> &&other)
      : uWS::TemplatedApp<SSL, CachingApp<SSL>>(std::move(other)) {
    // also move the cache
  }
  ...
  CachingApp &&get(const std::string &url,
                   uWS::MoveOnlyFunction<void(CachingHttpResponse *,
                                              uWS::HttpRequest *)> &&handler,
                   unsigned int secondsToExpiry) {
    ((uWS::TemplatedApp<SSL, CachingApp<SSL>> *)this)
        ->get(url, [this, handler = std::move(handler), secondsToExpiry](
                       auto *res, auto *req) mutable {
          /* We need to know the cache key and the time of now */
          std::string_view cache_key = req->getFullUrl();
          time_t now = static_cast<LoopData *>(
                           us_loop_ext((us_loop_t *)uWS::Loop::get()))
                           ->cacheTimepoint;
          ...
          if (it != cache.end()) {
            ...
            /* We are no longer valid, delete old cache and fall through to
             * create a new entry */
            delete it->second;

            // is the cache completed? if not, add yourself to the waiting list
            // of sockets to that cache
