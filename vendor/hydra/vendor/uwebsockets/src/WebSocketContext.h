...
struct WebSocketContext {
 ...
 private:
  ...
  WebSocketContext<SSL, isServer, USERDATA> *init() {
          ...
          if (!webSocketData->isShuttingDown) {
            ...
            if (webSocketContextData->closeHandler) {
              webSocketContextData->closeHandler(
                  ws, 1006, {(char *)reason, (size_t)code});
            }
    ...
    /* Handle FIN, WebSocket does not support half-closed sockets, so simply
     * close */
    us_socket_context_on_end(SSL, getSocketContext(), [](auto *s) {
      /* If we get a fin, we just close I guess */
      us_socket_close(SSL, (us_socket_t *)s, (int)ERR_TCP_FIN.length(),
                      (void *)ERR_TCP_FIN.data());
 ...
 public:
  ...
  static WebSocketContext *create(
      Loop * /*loop*/, us_socket_context_t *parentSocketContext,
