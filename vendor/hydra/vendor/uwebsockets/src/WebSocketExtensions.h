...
struct ExtensionsParser {
 ...
 public:
  ...
  int getToken(const char *&in, const char *stop) {
    ...
    static_assert(SHRT_MIN > INT_MIN,
                  "Integer overflow fix is invalid for this platform, report "
                  "this as a bug!");
...
static inline std::tuple<bool, int, int, std::string_view> negotiateCompression(
    bool wantCompression, int wantedCompressionWindow,
