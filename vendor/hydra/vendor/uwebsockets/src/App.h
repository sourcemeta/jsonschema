...
struct TemplatedApp {
 ...
 public:
  ...
  /* Server name */
  BuilderPatternReturnType &&addServerName(std::string hostname_pattern,
  ...
  BuilderPatternReturnType &&missingServerName(
      MoveOnlyFunction<void(const char *hostname)> handler) {
  ...
  BuilderPatternReturnType &&filter(
      MoveOnlyFunction<void(HttpResponse<SSL> *, int)> &&filterHandler) {
  ...
  TemplatedApp(SocketContextOptions options = {}) {
    ...
    this->any("/*", [](auto *res, auto * /*req*/) {
      ...
      res->end(
          "<html><body><h1>File Not Found</h1><hr><i>uWebSockets/20 "
          "Server</i></body></html>");
    });
  }
  ...
  template <typename UserData>
  BuilderPatternReturnType &&ws(std::string pattern,
  ...
  BuilderPatternReturnType &&options(
      std::string pattern,
      MoveOnlyFunction<void(HttpResponse<SSL> *, HttpRequest *)> &&handler) {
    if (httpContext) {
  ...
  BuilderPatternReturnType &&patch(
      std::string pattern,
      MoveOnlyFunction<void(HttpResponse<SSL> *, HttpRequest *)> &&handler) {
    if (httpContext) {
  ...
  BuilderPatternReturnType &&connect(
      std::string pattern,
      MoveOnlyFunction<void(HttpResponse<SSL> *, HttpRequest *)> &&handler) {
    if (httpContext) {
  ...
  BuilderPatternReturnType &&trace(
      std::string pattern,
      MoveOnlyFunction<void(HttpResponse<SSL> *, HttpRequest *)> &&handler) {
    if (httpContext) {
  ...
  /* Host, port, callback */
  BuilderPatternReturnType &&listen(
  ...
  BuilderPatternReturnType &&listen(
      std::string host, int port, int options,
      MoveOnlyFunction<void(us_listen_socket_t *)> &&handler) {
    if (!host.length()) {
  ...
  /* Port, callback */
  BuilderPatternReturnType &&listen(
      int port, MoveOnlyFunction<void(us_listen_socket_t *)> &&handler) {
  ...
  BuilderPatternReturnType &&listen(
      int options, MoveOnlyFunction<void(us_listen_socket_t *)> &&handler,
      std::string path) {
   ...
   * adoptSocket. */
  BuilderPatternReturnType &&preOpen(LIBUS_SOCKET_DESCRIPTOR (*handler)(
      struct us_socket_context_t *, LIBUS_SOCKET_DESCRIPTOR)) {
    httpContext->onPreOpen(handler);
    return std::move(static_cast<BuilderPatternReturnType &&>(*this));
  }

  BuilderPatternReturnType &&removeChildApp(BuilderPatternReturnType *app) {
    /* Remove this app from httpContextData list over child apps and reset round
     * robin */
  ...
  BuilderPatternReturnType &&addChildApp(BuilderPatternReturnType *app) {
    ...
    httpContext->onPreOpen(
        [](struct us_socket_context_t *context,
          ...
          // std::cout << "Distributing fd: " << fd << " from context: " <<
          // context << std::endl;

          unsigned int *roundRobin =
              &httpContext->getSocketContextData()->roundRobin;

          // std::cout << "Round robin is: " << *roundRobin << " and size of
          // apps is: " << httpContext->getSocketContextData()->childApps.size()
          // << std::endl;

          BuilderPatternReturnType *receivingApp =
              (BuilderPatternReturnType *)httpContext->getSocketContextData()
                  ->childApps[*roundRobin];

          // std::cout << "Loop is " << receivingApp->getLoop() << std::endl;

          receivingApp->getLoop()->defer([fd, receivingApp]() {
            // std::cout << "About to adopt socket " << fd << " on receivingApp
            // " << receivingApp << std::endl;
            receivingApp->adoptSocket(fd);
            // std::cout << "Done " << std::endl;
          });

          if (++(*roundRobin) ==
              httpContext->getSocketContextData()->childApps.size()) {
            *roundRobin = 0;
          }
