...
const std::string_view ERR_WEBSOCKET_TIMEOUT(
    "WebSocket timed out from inactivity");
...
const std::string_view ERR_INVALID_CLOSE_PAYLOAD(
    "Received invalid close payload");
const std::string_view ERR_PROTOCOL("Received invalid WebSocket frame");
const std::string_view ERR_TCP_FIN(
    "Received TCP FIN before WebSocket close frame");
...
static bool isValidUtf8(unsigned char *s, size_t length) {
  for (unsigned char *e = s + length; s != e;) {
    if (s + 16 <= e) {
      uint64_t tmp[2];
      memcpy(tmp, s, 16);
      if (((tmp[0] & 0x8080808080808080) | (tmp[1] & 0x8080808080808080)) ==
          0) {
        s += 16;
        continue;
      }
...
static inline CloseFrame parseClosePayload(char *src, size_t length) {
  ...
  if (length >= 2) {
    ...
    if (cf.code < 1000 || cf.code > 4999 ||
      ...
      return {1006, (char *)ERR_INVALID_CLOSE_PAYLOAD.data(),
              ERR_INVALID_CLOSE_PAYLOAD.length()};
    }
  }
  return cf;
}
...
struct WebSocketProtocol {
 ...
 protected:
  ...
  static inline bool consumeMessage(T payLength, char *&src,
                                    unsigned int &length,
