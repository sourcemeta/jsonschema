...
namespace uWS {
struct Http3Context {
  static Http3Context *create(us_loop_t *loop,
                              us_quic_socket_context_options_t options) {
    /* Create quic socket context (assumes h3 for now) */
    auto *context =
        us_create_quic_socket_context(loop, options, sizeof(Http3ContextData));

    /* Specify application callbacks */
    us_quic_socket_context_on_stream_data(
        context, [](us_quic_stream_t *s, char *data, int length) {
          Http3ResponseData *responseData =
              (Http3ResponseData *)us_quic_stream_ext(s);
    ...
    us_quic_socket_context_on_stream_end(context, [](us_quic_stream_t *s) {
      ...
      // if (responseData->buffer.length() && (responseData->bufferOffset ==
      // (int) responseData->buffer.length())) {
      //     printf("We got FIN and we have no backpressure, closing stream
      //     now!\n");
      //     //us_quic_stream_close(s);
      // } else {
      //     //printf("We got FIN but we have data to write, so keeping
      //     connection half-closed!\n");
      // }
    });
    us_quic_socket_context_on_stream_open(
        context, [](us_quic_stream_t *s, int is_client) {
          printf("Stream open!\n");
    ...
    us_quic_socket_context_on_stream_writable(context, [](us_quic_stream_t *s) {
      ...
      if (responseData->onWritable) {
        ...
        int written =
            us_quic_stream_write(s, (char *)responseData->backpressure.data(),
    ...
    us_quic_socket_context_on_stream_headers(context, [](us_quic_stream_t *s) {
      ...
      Http3ContextData *contextData =
          (Http3ContextData *)us_quic_socket_context_ext(
              us_quic_socket_context(us_quic_stream_socket(s)));
      ...
      contextData->router.getUserData() = {(Http3Response *)s,
                                           (Http3Request *)nullptr};
      contextData->router.route(upperCasedMethod, path);
    });
    us_quic_socket_context_on_open(context,
                                   [](us_quic_socket_t *s, int is_client) {
  ...
  us_quic_listen_socket_t *listen(const char *host, int port) {
    ...
    us_quic_listen_socket_t *listen_socket =
        us_quic_socket_context_listen((us_quic_socket_context_t *)this, host,
                                      port, sizeof(Http3ResponseData));
  ...
  void onHttp(std::string method, std::string path,
              MoveOnlyFunction<void(Http3Response *, Http3Request *)> &&cb) {
    // modifies the router we own as part of Http3ContextData, used in callbacks
    // set in init
    ...
    contextData->router.add(
        methods, path,
        [handler = std::move(cb)](
            HttpRouter<Http3ContextData::RouterData> *router) mutable {
