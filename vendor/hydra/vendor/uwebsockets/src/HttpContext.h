...
struct HttpContext {
        ...
        SSL, getSocketContext(), [](us_socket_t *s, char *data, int length) {
          ...
          auto [err, returnedSocket] = httpResponseData->consumePostPadded(
              ...
              [httpContextData](void *s, HttpRequest *httpRequest) -> void * {
                 ...
                 * connection. Important for denying async pipelining until, if
                 * ever, we want to suppot it. Otherwise requests can get mixed
                 * up on the same connection. We still support sync pipelining.
                 */
 ...
 public:
  ...
  void onHttp(
      std::string method, std::string pattern,
  ...
  void onPreOpen(LIBUS_SOCKET_DESCRIPTOR (*handler)(
