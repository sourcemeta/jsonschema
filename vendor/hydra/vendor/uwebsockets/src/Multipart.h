...
/* Implements the multipart protocol. Builds atop parts of our common http
 * parser (not yet refactored that way). */
...
struct ParameterParser {
  /* Takes the line, commonly given as content-disposition header in the
   * multipart */
  ParameterParser(std::string_view line) { remainingLine = line; }
    ...
    if (!remainingLine.length()) {
      ...
      if (remainingLine[0] == ';' || remainingLine[0] == '=') {
        ...
        if (remainingLine[0] == '\"') {
          ...
          int tokenLength = 0;
          while (remainingLine.length() && remainingLine[0] != ';' &&
                 remainingLine[0] != '=' && !isspace(remainingLine[0])) {
            remainingLine.remove_prefix(1);
            tokenLength++;
          }
...
struct MultipartParser {
  ...
  MultipartParser(std::string_view contentType) {
    ...
    if (contentType.length() < 10 ||
        contentType.substr(0, 10) != "multipart/") {
      return;
    }

    /* For now we simply guess boundary will lie between = and end. This is not
     * entirely standards compliant as boundary may be expressed with or without
     * " and spaces */
  ...
  bool isValid() { return prependedBoundary.length() != 0; }

  /* Set the body once, before getting any parts */
  void setBody(std::string_view body) { remainingBody = body; }

  /* Parse out the next part's data, filling the headers. Returns nullopt on end
   * or error. */
  std::optional<std::string_view> getNextPart(
    ...
    /* We are allowed to post pad like this because we know the boundary is at
     * least 2 bytes */
    /* This makes parsing a second pass invalid, so you can only iterate over
     * parts once */
    memset((char *)part.data() + part.length(), '\r', 1);

    /* For this to be a valid part, we need to consume at least 4 bytes
     * (\r\n\r\n) */
    int consumed = getHeaders((char *)part.data(),
                              (char *)part.data() + part.length(), headers);
