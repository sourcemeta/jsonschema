...
struct Http3Response {
  // this one is AsyncSocket, so it has to translate to the stream - abrupt
  // stream termination
  void close() {
  ...
  void endWithoutBody(
      std::optional<size_t> reportedContentLength = std::nullopt,
      bool closeConnection = false) {}

  Http3Response *writeStatus(std::string_view status) {
    Http3ResponseData *responseData =
        (Http3ResponseData *)us_quic_stream_ext((us_quic_stream_t *)this);

    /* Nothing is done if status already written */
    if (responseData->headerOffset == 0) {
      us_quic_socket_context_set_header(nullptr, 0, (char *)":status", 7,
  ...
  Http3Response *writeHeader(std::string_view key, std::string_view value) {
    ...
    us_quic_socket_context_set_header(nullptr, responseData->headerOffset++,
                                      key.data(), key.length(), value.data(),
                                      value.length());
  ...
  std::pair<bool, bool> tryEnd(std::string_view data, uintmax_t totalSize = 0) {
    ...
    us_quic_socket_context_send_headers(nullptr, (us_quic_stream_t *)this,
                                        responseData->headerOffset,
                                        data.length() > 0);

    unsigned int written = us_quic_stream_write(
        (us_quic_stream_t *)this, (char *)data.data(), (int)data.length());
  ...
  /* Idnetical */
  Http3Response *write(std::string_view data) { return this; }
  ...
  void end(std::string_view data = {}, bool closeConnection = false) {
    ...
    /* Buffer up remains */
    if (written != data.length()) {
      responseData->backpressure.append(data.data() + written,
                                        data.length() - written);
    } else {
      /* Every request has its own stream, so we conceptually serve requests
       * like in HTTP 1.0 */
   ...
   * last segment. */
  Http3Response *onData(
      MoveOnlyFunction<void(std::string_view, bool)> &&handler) {
    Http3ResponseData *responseData =
        (Http3ResponseData *)us_quic_stream_ext((us_quic_stream_t *)this);
