...
/* A timeout granularity of 4 seconds means give or take 4 seconds from set
 * timeout */
...
int us_udp_socket_receive(struct us_udp_socket_t *s,
                          struct us_udp_packet_buffer_t *buf);

void us_udp_buffer_set_packet_payload(struct us_udp_packet_buffer_t *send_buf,
                                      int index, int offset, void *payload,
                                      int length, void *peer_addr);

int us_udp_socket_send(struct us_udp_socket_t *s,
                       struct us_udp_packet_buffer_t *buf, int num);

/* Allocates a packet buffer that is reuable per thread. Mutated by
 * us_udp_socket_receive. */
struct us_udp_packet_buffer_t *us_create_udp_packet_buffer();

/* Creates a (heavy-weight) UDP socket with a user space ring buffer. Again,
 * this one is heavy weight and shoud be reused. One entire QUIC server can be
 * implemented using only one single UDP socket so weight is not a concern as is
 * the case for TCP sockets which are 1-to-1 with TCP connections. */
// struct us_udp_socket_t *us_create_udp_socket(struct us_loop_t *loop, void
// (*read_cb)(struct us_udp_socket_t *), unsigned short port);

// struct us_udp_socket_t *us_create_udp_socket(struct us_loop_t *loop, void
// (*data_cb)(struct us_udp_socket_t *, struct us_udp_packet_buffer_t *, int),
// void (*drain_cb)(struct us_udp_socket_t *), char *host, unsigned short port);

struct us_udp_socket_t *us_create_udp_socket(
    struct us_loop_t *loop, struct us_udp_packet_buffer_t *buf,
...
int us_udp_socket_bind(struct us_udp_socket_t *s, const char *hostname,
                       unsigned int port);
...
void us_timer_set(struct us_timer_t *timer, void (*cb)(struct us_timer_t *t),
...
struct us_socket_context_options_t {
  ...
  int ssl_prefer_low_memory_usage; /* Todo: rename to prefer_low_memory_usage
                                      and apply for TCP as well */
};

/* Return 15-bit timestamp for this context */
unsigned short us_socket_context_timestamp(int ssl,
                                           struct us_socket_context_t *context);

/* Adds SNI domain and cert in asn1 format */
void us_socket_context_add_server_name(
    int ssl, struct us_socket_context_t *context, const char *hostname_pattern,
    struct us_socket_context_options_t options, void *user);
void us_socket_context_remove_server_name(int ssl,
                                          struct us_socket_context_t *context,
                                          const char *hostname_pattern);
void us_socket_context_on_server_name(int ssl,
                                      struct us_socket_context_t *context,
                                      void (*cb)(struct us_socket_context_t *,
                                                 const char *hostname));
...
void *us_socket_context_get_native_handle(int ssl,
                                          struct us_socket_context_t *context);

/* A socket context holds shared callbacks and user data extension for
 * associated sockets */
struct us_socket_context_t *us_create_socket_context(
    int ssl, struct us_loop_t *loop, int ext_size,
...
void us_socket_context_on_open(
    int ssl, struct us_socket_context_t *context,
    struct us_socket_t *(*on_open)(struct us_socket_t *s, int is_client,
                                   char *ip, int ip_length));
void us_socket_context_on_close(
    int ssl, struct us_socket_context_t *context,
    struct us_socket_t *(*on_close)(struct us_socket_t *s, int code,
                                    void *reason));
void us_socket_context_on_data(
    int ssl, struct us_socket_context_t *context,
    struct us_socket_t *(*on_data)(struct us_socket_t *s, char *data,
                                   int length));
...
/* Closes all open sockets, including listen sockets. Does not invalidate the
 * socket context. */
void us_socket_context_close(int ssl, struct us_socket_context_t *context);

/* Listen for connections. Acts as the main driving cog in a server. Will call
 * set async callbacks. */
struct us_listen_socket_t *us_socket_context_listen(
    int ssl, struct us_socket_context_t *context, const char *host, int port,
    int options, int socket_ext_size);

struct us_listen_socket_t *us_socket_context_listen_unix(
    int ssl, struct us_socket_context_t *context, const char *path, int options,
    int socket_ext_size);

/* listen_socket.c/.h */
void us_listen_socket_close(int ssl, struct us_listen_socket_t *ls);

/* Adopt a socket which was accepted either internally, or from another accept()
 * outside libusockets */
struct us_socket_t *us_adopt_accepted_socket(
    int ssl, struct us_socket_context_t *context,
...
struct us_socket_t *us_socket_context_connect(
    int ssl, struct us_socket_context_t *context, const char *host, int port,
    const char *source_host, int options, int socket_ext_size);

struct us_socket_t *us_socket_context_connect_unix(
    int ssl, struct us_socket_context_t *context, const char *server_path,
    int options, int socket_ext_size);
 ...
 * like us_socket_close but does not trigger on_close event since you never got
 * the on_open event first. */
struct us_socket_t *us_socket_close_connecting(int ssl, struct us_socket_t *s);

/* Returns the loop for this socket context. */
struct us_loop_t *us_socket_context_loop(int ssl,
                                         struct us_socket_context_t *context);
...
struct us_socket_t *us_socket_context_adopt_socket(
    int ssl, struct us_socket_context_t *context, struct us_socket_t *s,
    int ext_size);

/* Create a child socket context which acts much like its own socket context
 * with its own callbacks yet still relies on the parent socket context for some
 * shared resources. Child socket contexts should be used together with socket
 * adoptions and nothing else. */
struct us_socket_context_t *us_create_child_socket_context(
    int ssl, struct us_socket_context_t *context, int context_ext_size);
...
/* Blocks the calling thread and drives the event loop until no more
 * non-fallthrough polls are scheduled */
void us_loop_run(struct us_loop_t *loop);

/* Signals the loop from any thread to wake up and execute its wakeup handler
 * from the loop's own running thread. This is the only fully thread-safe
 * function and serves as the basis for thread safety */
 ...
 * Will call the on_writable callback of active socket context on failure to
 * write everything off in one go. Set hint msg_more if you have more immediate
 * data to write. */
int us_socket_write(int ssl, struct us_socket_t *s, const char *data,
                    int length, int msg_more);

/* Special path for non-SSL sockets. Used to send header and payload in one go.
 * Works like us_socket_write. */
int us_socket_write2(int ssl, struct us_socket_t *s, const char *header,
                     int header_length, const char *payload,
                     int payload_length);

/* Set a low precision, high performance timer on a socket. A socket can only
 * have one single active timer at any given point in time. Will remove any such
 * pre set timer */
void us_socket_timeout(int ssl, struct us_socket_t *s, unsigned int seconds);

/* Set a low precision, high performance timer on a socket. Suitable for
 * per-minute precision. */
void us_socket_long_timeout(int ssl, struct us_socket_t *s,
...
struct us_socket_t *us_socket_close(int ssl, struct us_socket_t *s, int code,
                                    void *reason);
...
void us_socket_remote_address(int ssl, struct us_socket_t *s, char *buf,
                              int *length);
...
#if !defined(LIBUS_USE_IO_URING) && !defined(LIBUS_USE_EPOLL) && \
    !defined(LIBUS_USE_LIBUV) && !defined(LIBUS_USE_GCD) &&      \
    !defined(LIBUS_USE_KQUEUE) && !defined(LIBUS_USE_ASIO)
#if defined(_WIN32)
