 ...
 */
void Curl_failf(struct Curl_easy *data, const char *fmt, ...) CURL_PRINTF(2, 3);
...
#define infof(data, ...)                                          \
  do {                                                            \
    if (Curl_trc_is_verbose(data)) Curl_infof(data, __VA_ARGS__); \
  } while (0)
...
#define Curl_trc_cf_is_verbose(cf, data) \
  (Curl_trc_is_verbose(data) && (cf) &&  \
   (cf)->cft->log_level >= CURL_LOG_LVL_INFO)
#define Curl_trc_ft_is_verbose(data, ft) \
  (Curl_trc_is_verbose(data) && (ft)->log_level >= CURL_LOG_LVL_INFO)

/**
 * Output an informational message when transfer's verbose logging is enabled.
 */
void Curl_infof(struct Curl_easy *data, const char *fmt, ...) CURL_PRINTF(2, 3);
...
static void Curl_trc_cf_infof(struct Curl_easy *data, struct Curl_cfilter *cf,
                              const char *fmt, ...) {
  (void)data;
  (void)cf;
  (void)fmt;
...
static void Curl_trc_ft_infof(struct Curl_easy *data, struct curl_trc_feat *ft,
                              const char *fmt, ...) {
  (void)data;
  (void)ft;
  (void)fmt;
