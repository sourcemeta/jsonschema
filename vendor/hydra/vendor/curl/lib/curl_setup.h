...
/* Workaround for Homebrew gcc 12.4.0, 13.3.0, 14.1.0 and newer (as of 14.1.0)
   ...
   to enable the problematic attributes. This makes the feature check fail. */
    ...
    __GNUC__ >= 12 && defined(__has_attribute)
...
#if defined(TARGET_OS_MAC) && TARGET_OS_MAC && defined(TARGET_OS_OSX) &&   \
    !TARGET_OS_OSX && (!defined(TARGET_OS_IPHONE) || !TARGET_OS_IPHONE) && \
...
#if defined(__APPLE__) &&                                        \
    ((!defined(__apple_build_version__) && defined(__clang__) && \
      __clang_major__ < 17) ||                                   \
